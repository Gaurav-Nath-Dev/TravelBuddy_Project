    ...
    <nav className="mt-8 bg-white text-2xl text-gray-700 font-medium m-auto flex justify-between items-center w-[95%]">
      ...
      :
        <ul className={"gap-20 "+(openNav ? "hidden":"flex" )}>
          ...
          <li className="hover:border-b-2 hover:border-blue_main py-3"><Link to={"/community"}>Community</Link></li>
        </ul> 
      }
      </div>
      ...
      {isOpen && (
        <div
          className="absolute right-0 z-10 mt-2 mr-9 w-56 origin-top-right rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
        >
          ...
          <>
            <div className="flex  gap-4">
              <button className="hover:border-blue_main border-2 border-gray-200 border-solid py-2 px-4 rounded-full duration-500"><Link to={"/login"}>Log In</Link></button>
              <button className="hover:border-blue_main border-2 border-gray-200 border-solid py-2 px-4 rounded-full duration-500"><Link to={"/signup"}>Sign up</Link></button>
            </div>
          </>
        }
        </div>
